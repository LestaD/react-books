import React, { PropTypes, Component } from 'react'
import ReactDOM from 'react-dom'
import * as StyledComponents from 'styled-components'
import { transform } from 'babel-standalone'
import gro from 'gro'

import * as PatternLibrary from '@ui/pattern-library'
import * as Atoms from '@ui/pattern-library/atoms'
import * as Molecules from '@ui/pattern-library/molecules'
// import * as Organisms from '@ui/pattern-library/organisms'
import * as Themes from '@ui/pattern-library/themes'

import CodeMirror from 'codemirror'
import 'codemirror/mode/javascript/javascript.js'
import 'codemirror/mode/xml/xml.js'
import 'codemirror/mode/jsx/jsx.js'
import 'codemirror/lib/codemirror.css'
import 'codemirror/theme/mdn-like.css'

const { default: styled, ThemeProvider } = StyledComponents

const syntheticScope = {
  'styled-components': StyledComponents,
  react: React,
  '@ui/pattern-library': PatternLibrary,
  '@ui/pattern-library/atoms': Atoms,
  '@ui/pattern-library/molecules': Molecules,
  // '@ui/pattern-library/organisms': Organisms,
  '@ui/pattern-library/themes': Themes,
}

function syntheticRequire(name) {
  if (!syntheticScope[name]) {
    throw new Error(`Cannot find module '${name}'`)
  }

  return syntheticScope[name]
}

const DEBOUNCE_TIME = 900 // ms

const Wrapper = styled.div`
  display: flex;
  flex-flow: column nowrap;
  font-size: 13px;

  & .cm-s-mdn-like.CodeMirror {
    border: 1px solid #ccc;
    border-left: none;
  }

  & .CodeMirror {
    height: auto;
  }
`

const RenderedWrapper = styled.div`
  padding: 20px;
  border: 1px solid #ccc;
  border-bottom: none;
  border-left: 6px solid rgba(0,83,159,0.65);
  ${''/* background-color: white; */}
  background-image: url(data:image/png;base64,);
`

const ErrorWrapper = styled.pre`
  font-size: 12px;
  line-height: 2;
  padding: 20px;
  background-color: #ef3737;
  color: white;
  margin-bottom: 0;
  white-space: pre-wrap;
`

const gr = gro('Playground')

export default class Playground extends Component {
  static propTypes = {
    sourceCode: PropTypes.string,
  }

  static defaultProps = {
    sourceCode: '',
  }

  state = {
    error: false,
  }

  editor = null
  timeoutID = null
  container = null

  shouldComponentUpdate(nextProps, nextState) {
    if (this.state.error !== nextState.error) {
      return true
    }

    return false
  }

  componentWillUnmount() {
    if (this.editor) {
      this.editor.off('change', this.handleCodeChange)
      this.editor.toTextArea()
    }
    if (this.container) {
      this.container.componentWillUnmount()
      ReactDOM.unmountComponentAtNode(this.refs.mountPoint)
    }
  }

  componentDidMount() {
    this.editor = CodeMirror.fromTextArea(this.refs.editor, {
      mode: 'jsx',
      lineNumbers: true,
      smartIndent: false,
      tabSize: 2,
      matchBrackets: true,
      theme: 'mdn-like',
      readOnly: false,
      fontSize: 12,
      viewportMargin: Infinity,
    })

    this.editor.on('change', this.handleCodeChange)
    this.handleCodeChange()
  }

  handleCodeChange = () => {
    clearTimeout(this.timeoutID)
    this.timeoutID = setTimeout(() => this.executeCode(), DEBOUNCE_TIME)
  }

  compileSourceCode = (code) => {
    const functionCode = transform(code, {
      presets: ['es2015', 'stage-0', 'react'],
    }).code

    return `
      (function(require, module, exports, mountPoint) {
        ${functionCode}
      })
    `
  }

  executeCode() {
    const gend = gr.group('executeCode')
    const time = Date.now()

    const mountPoint = this.refs.mountPoint
    const mod = { exports: {} }

    try {
      ReactDOM.unmountComponentAtNode(mountPoint)
      this.setState({ error: false })
    }
    catch (error) {
      console.error('Unmounting error:', error)
      this.setState({ error })
    }

    try {
      const ce = gro.group('compileEval')
      const dc = gro.time('compileCode')
      const compiledFunction = this.compileSourceCode(this.editor.getValue())
      dc()

      const ec = gro.time('evalCode')
      // eslint-disable-next-line no-eval
      const result = eval(compiledFunction)(syntheticRequire, mod, mod.exports, mountPoint)
      ec()
      ce()

      const Element = mod.exports.default
      Element.displayName = 'CompiledPlaygroundComponent'

      const rr = gro.time('render')
      this.container = ReactDOM.render(
        <ThemeProvider theme={Themes.SphereTheme}>
          <RenderedWrapper>
            <Element />
          </RenderedWrapper>
        </ThemeProvider>,
        mountPoint,
      )
      rr()

      this.setState({ error: false })
    }
    catch (error) {
      console.timeEnd('evalCode')
      console.error('Playground error:', error)
      this.setState({ error })
    }
    console.log(`Total time: ${Date.now() - time}ms`)
    gend()
  }

  render() {
    return (
      <Wrapper>
        {this.state.error && <ErrorWrapper>{this.state.error.message}</ErrorWrapper>}
        <div ref="mountPoint" />
        <textarea ref="editor" defaultValue={this.props.sourceCode} />
      </Wrapper>
    )
  }
}
